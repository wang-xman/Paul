...
from linear_space.algebra.functions import transpose
...
class Test_Transpose(unittest.TestCase):
    def test_column_vector(self):
        ...
        self.assertTrue(res.get_dim(), 2)
        self.assertTrue(res.nrows, 1)
        self.assertTrue(res.ncols, 5)
    ...
    def test_matrix(self):
        array_a = np.array([[1.0, 1.0j, 2.0, 2.0j], [1.0, 1.0j, 2.0, 2.0j]])
        mat = Matrix(array=array_a)
        res = transpose(mat)
        self.assertTrue(isinstance(res, Matrix))
        self.assertTrue(res.get_dim(), 2)
        self.assertTrue(res.nrows, 4)
        self.assertTrue(res.ncols, 2)
        self.assertEqual(res[2][0], 2)
        self.assertEqual(res[2][1], 2)
        self.assertEqual(res[3][0], 2j)
        self.assertEqual(res[3][1], 2j)
    ...
    def test_id_matrix(self):
        row_size = 3
        mat = IdentityMatrix(row_size=row_size)
        res = transpose(mat)
        self.assertTrue(isinstance(res, IdentityMatrix))
        self.assertEqual(mat.trace, 3)
