...
test_gate = singles['Ry']


class TestRyGate_ZeroState(unittest.TestCase):
...
class TestRyGate_DefaultTheta(unittest.TestCase):
...
class TestRyGate_Pi(unittest.TestCase):
    """ Matrix is [[0 -1], [1, 0]] """
    ...
    def test_phase_up_state(self):
        ...
        expected = qubit_from_bitlist([(1, '1')])
...
class TestRyGate_HalfPi(unittest.TestCase):
    theta = 0.5 * np.pi
    ...
    def test_phase_down_state(self):
        ...
        expected = qubit_from_bitlist([(1.0 * self.cos_qp, '1'),
                                       (-1.0 * self.sin_qp, '0')])
        self.assertTrue(new_state == expected)
